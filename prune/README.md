...
- **Stale knowledge base embeddings are reconciled** — removing a file or deleting a folder inside a knowledge base drops the link but, in released Open WebUI versions, leaves the file's chunks in the knowledge base collection, so answers keep citing it. Each sweep compares every kept knowledge base's collection against its linked files and deletes the chunks of files it no longer lists (files inside the grace window are left alone, an upload is embedded before it is linked).

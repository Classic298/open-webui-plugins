...
- **Manual admin page** — a session-gated page at `/prune` (admins only, anyone else is redirected to `/`) mirroring the valve sections, with a hoverable ⓘ explanation on every option, a **Preview** that shows exactly what would be deleted, and an **Execute** with typed confirmation and a live run log. VACUUM lives here as an explicit per-run maintenance checkbox.
...
- **Manual** — open `https://your-instance/prune` as an admin. Configure options (every row has a hoverable ⓘ explanation), hit **Preview** for a per-category count, then **Execute** (type `DELETE` to confirm) and watch the live run log. Manual runs and automatic passes share one lock, so they can never overlap.

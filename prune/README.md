...
- **Stale knowledge base embeddings are reconciled** — deleting a folder inside a knowledge base together with its contents drops the links but, in released Open WebUI versions, leaves the files' chunks in the knowledge base collection, so answers keep citing them. Each sweep compares every kept knowledge base's collection against its linked files and deletes the chunks of files it no longer lists. Files touched inside `orphan_file_grace_hours` are left alone, because a file is embedded before it is linked.
...
- **Manual** — open `https://your-instance/prune` as an admin. Configure options (every row has a hoverable ⓘ explanation), hit **Preview** for a per-category count, expand any nonzero row to inspect the exact records behind it (paginated, exportable as JSON), then **Execute** (type `DELETE` to confirm) and watch the progress bar and live run log. A **Cancel** button appears while a preview or execute is running: it stops the pass at the next batch boundary (never mid-transaction), so anything already deleted stays deleted and re-running is safe. Both run in the background with a stage-by-stage progress bar, so neither blocks the browser and a reopened page resumes watching the current run. The **Speed limits** section caps scan and deletion rates for that run only (empty = valve settings). Manual runs and automatic passes share one lock, so they can never overlap.

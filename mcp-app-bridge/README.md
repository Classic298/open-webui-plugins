...
> [!NOTE]
> **Same-origin toggle:** the app stays isolated even when "iframe Sandbox Allow Same Origin" is enabled. It always runs in its own nested frame, so it cannot read Open WebUI's cookies, local storage or page, and it gets no cookies or local storage of its own either. Before 0.8.0, with the toggle on, the app ran on Open WebUI's origin and could read its cookies and local storage. Now only the bridge's own host frame runs there.
